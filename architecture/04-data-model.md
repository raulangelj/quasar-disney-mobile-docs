...
User 1 ── 1 Session
Title
Carousel * ── * Title          (ordered membership; HomeFeed only)
HomeFeed page  = 1 hero Carousel + N other Carousels
ContinueWatching page  = ordered Title items with progress fields
```

**Client-composed home (not an API resource):**

```
[ HomeFeed.hero ] + [ ContinueWatching as a row ] + [ HomeFeed.carousels… ]
```

Continue watching sits **directly under hero** and shifts the other HomeFeed carousels down one. Storefront owns that merge (ADR-0006).
...
| **Title** | Catalog item a tile represents | Shared across carousel variants. Artwork is a map on Title, not its own entity |
| **Carousel** | A HomeFeed row: display title, variant, ordered Titles | Config-driven variants stay config, not types |
| **HomeFeed** | Paginated page of one **hero** + other carousels | First page: **hero + 15 carousels**. Not a stored aggregate |
| **ContinueWatching** | Paginated list of Titles **with progress fields** | Separate authenticated resource. Not a HomeFeed carousel |

**Not entities:** Credentials (login request DTO), Artwork (URIs on Title), WatchProgress (fields on each CW item — see §1.3), LiveBroadcast (deferred; later feature), Profile, Download, Search, Payment, pagination cursors (wire only).

### 1.2 Title (closes OQ-02)

| Field | Type | Notes |
|-------|------|--------|
| `id` | UUID string | Public; tiles, later details, CW merge |
| `title` | string | Shown on tap-alert; CW and landscape rows |
| `artwork` | `Partial<Record<AspectRatio, uri>>` | Keys: `'2:3'` (portrait), `'16:9'` (CW / landscape), `'3:4'` (hero). Only ratios a variant needs |
...
### 1.3 ContinueWatching item

Not a separate table. Each CW element is a **Title plus**:
...
The Continue Watching **request sends the JWT**. The mock (and later the backend) keys progress off that session. There is no WatchProgress id.

### 1.4 Carousel and HomeFeed

| Field | Type | Notes |
|-------|------|--------|
| `id` | UUID string | Row identity for paging tiles |
| `displayTitle` | string | Row header; i18n in the client |
| `variant` | enum | HomeFeed: `'hero' \| 'standardPortrait' \| 'standardLandscape'`. **`'continueWatching'` is not a HomeFeed variant** — CW is the other endpoint. `'live'` is out until that feature lands |
| `items` | Title[] | One page of tiles; `nextCursor` on the page envelope |
| `nextCursor` | string \| null | Opaque; Phase 1 mocks may send `null` |

HomeFeed first page: `hero` (one Carousel, variant `hero`) + `carousels` (15). Further vertical pages, if any, are more `carousels` only (no second hero). Phase 1 mocks may set HomeFeed `nextCursor` to `null`.
...
| **Title, Carousel, HomeFeed** | API module (wire + fixtures) | Storefront content slice (in-memory copy) | Storefront never authors catalog |
| **ContinueWatching** | API module (JWT-authenticated fixtures) | Storefront content slice | Progress is on the items, not in Auth |
| **Composed home list** | Storefront feature | — | Inserts CW under hero (ADR-0006) |
...
| **HomeFeed, ContinueWatching, Title** | Memory only | content slice; overwritten by fetches |
| **Artwork files** | App bundle | placeholder-art assets; Title stores URIs |
...
| **Title, Carousel** | UUID v4 strings | Yes |
| **ContinueWatching item** | Title `id` (plus implicit session) | No separate id |
...
| Title, Carousel, artwork URIs | Internal | In memory / binary | Next fetch or next app release |
...
| 2 | HomeFeed (hero + 15 carousels) | Same gate as `/me` | Same loader |
| 3 | ContinueWatching | Same gate as `/me`; **again** whenever the storefront screen is shown | Cold start: loader. Later visits: **silent** (stale-while-revalidate; no full-screen loader) |
...
**Silent CW reload:** on storefront appearing again (tab/focus), refetch ContinueWatching only; replace that row. Hero and the 15 stay.
...
| Continue watching while home is already shown | **Stale-while-revalidate** on that row only |
...
| 2 | Artwork | Map of aspect-ratio → URI on **Title** | Fixtures, not a DAM | Artwork as its own entity |
| 3 | Live | **Out** of this model | Later feature | LiveBroadcast / `'live'` variant in Phase 1 data |
| 4 | Catalog split | **HomeFeed** (hero + 15) and **ContinueWatching** as two JWT-authenticated GETs; client inserts CW under hero | Matches how a real catalog + continue-watching API split; CW can silent-reload alone | One home payload; CW as a HomeFeed variant |
...
|----|----------|-------|------------|
| OQ-22 | JSON names and paths for `/me`, HomeFeed, ContinueWatching, and the page envelope (`nextCursor` vs `next`) | Mobile | 1.11 Interface Contracts |
| OQ-23 | Tiles per carousel horizontal page (first-page size for items inside a row) | Mobile | 1.11; storefront STEP |
...
Carried forward: OQ-10 (backend team accepts the contract → 1.11), OQ-17 (mock strategy → 1.11). **OQ-02** (card schema), **OQ-19** (cursor vs offset), and HomeFeed first-page size from **OQ-20** (hero + 15; CW separate) are **closed** here. Horizontal tile page size remains as OQ-23.
