...
┌─────────────────────────────────────────────────────────────────┐
│                         App shell                               │
│         RN host · navigation · store · persist rehydrate        │
│                                                                 │
│         ┌──────────────┐         ┌──────────────────┐           │
...
|------|----------------|------|
| **App shell** | Boots the RN host on iOS/Android: navigation, store/middleware registration, theme provider, persist rehydrate before choosing the auth vs app navigator | Bare RN, TypeScript, React Navigation, Redux Toolkit store composition |
...
| **Storefront feature** | Home/browse and config-driven carousels. Owns the content slice (not persisted). Imports `shared/` only | RTK slice, custom hooks, Styled Components |
...
| Auth / Storefront → API module (via middleware only) | Login, home/carousel payloads | Async (Promises; mock latency/failure) | API module owns wire shapes; features own slice state | **Yes** — TS interfaces, enums, envelope, error shape | Transport is **axios**, not `fetch`. Screens/hooks never import `axios`. This is the swap boundary |
...
1. App shell boots. **redux-persist** rehydrates the **auth slice only** from **Keychain (iOS) / Keystore (Android)**.
2. If a session exists, shell skips Welcome and mounts the app navigator with the **dark** theme.
3. If not, shell applies the **light** theme and mounts the unauthenticated navigator.
4. Auth shows Welcome (Shared atoms). CTA → email → password (two-step, Phase 1a).
5. Submit: Auth dispatches → middleware → **API module** (axios instance or mock). No screen imports `axios`.
6. **Success:** mock returns an opaque token → auth slice stores it → persist writes it to secure storage → shell reads `isAuthenticated` → switches to the app navigator and **dark** theme.
7. **Failure:** mock fails on demand → same error shape as the future API → credentials screen renders the reference inline error (red underline + message). Not an alert, not a local `if (password !== …)` branch.

### Flow 2 — Storefront load and card tap

1. Shell mounts Storefront as the home tab (other tabs inert).
2. Storefront dispatches load-home → middleware → API module → mock payload (carousel configs + tiles).
3. Storefront renders the config-driven carousel using Shared tiles; it does not import Auth.
...
| HTTP + persistence | **Buy (OSS)** | axios, Redux Toolkit, redux-persist, Keychain/Keystore wrapper (`react-native-encrypted-storage` or a thin Keychain adapter) |
...
**Hard dependencies:** React Native, Redux Toolkit, React Navigation, Styled Components, axios, redux-persist, secure-storage library. No cloud vendor lock-in in Phase 1.
...
| Shared → `features/` | **No** |
| Persist | **Auth slice only**, into platform secure storage — not AsyncStorage, not the rest of the store |
...
| 7 | Session persistence | redux-persist, auth slice, Keychain/Keystore from day one | Real JWT later is a payload change, not a storage rewrite | AsyncStorage for tokens; persisting the whole store |
...
|----|----------|-------|------------|
| OQ-16 | Persist backend library: `react-native-encrypted-storage` vs a thin `react-native-keychain` adapter | Mobile | 1.3a Native App; foundation STEP |
